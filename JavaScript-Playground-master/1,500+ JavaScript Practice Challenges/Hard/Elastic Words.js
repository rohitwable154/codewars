...
function elasticize(word) {
	...
	if (word.length % 2 === 1) {
		return arr.map((item, index) => index <= center ? item.repeat(index + 1) : item.repeat(center * 2 - index + 1)).join('');
	} else {
		return arr.map((item, index) => index < center ? item.repeat(index + 1) : item.repeat(center * 2 - index)).join('');
	}
}
...
const elasticize=word=>	word.replace(/\w/g,(e,i,a)=>i<a.length/2?e.repeat(i+1):e.repeat(a.length-i))
...
const elasticize = word => {
	const ceil = Math.ceil(word.length / 2);
	const floor = Math.floor(word.length / 2);
	return word.length < 3 ? word : `${[...word.slice(0, ceil)].map((char, i) => char.repeat(1 + i)).join``}${[...word.slice(ceil)].map((char, i) => char.repeat(floor - i)).join``}`;
}