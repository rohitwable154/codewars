...
function alphaClash(str_A, ind_A, str_Z, ind_Z) {
    const code_A = [...str_A].filter((_, i) => !ind_Z.includes(i)).map(s => s.charCodeAt());
    const code_Z = [...str_Z].filter((_, i) => !ind_A.includes(i)).map(s => s.charCodeAt());
    return {
        A: code_A.reduce((a, c, i) => a + Math.max(c - code_Z[i], 0), 0),
        Z: code_Z.reduce((a, c, i) => a + Math.max(c - code_A[i], 0), 0)
    };
}
...
function alphaClash(str_A, ind_A, str_Z, ind_Z) {
    let s = str_A.split('').filter((a, i) => !ind_Z.includes(i))
    let t = str_Z.split('').filter((z, i) => !ind_A.includes(i))
    let first = s.map(a => a.charCodeAt())
    let second = t.map(a => a.charCodeAt())
    let val = first.map((a, i) => a > second[i] ? {
        A: a - second[i]
    } : { Z: second[i] - a })
    return val.reduce((ac, b) => {
...
const alphaClash = (str_A, ind_A, str_Z, ind_Z) => {
    str_A = [...str_A]
        .map((c, i) => ind_Z.includes(i) ? "" : c.charCodeAt(0))
        .filter(Boolean);
    str_Z = [...str_Z]
        .map((c, i) => ind_A.includes(i) ? "" : c.charCodeAt(0))
        .filter(Boolean);
    let scr = { A: 0, Z: 0 };
    str_A.forEach((val, idx) => {
        if (val > str_Z[idx]) scr.A += val - str_Z[idx];
        if (val < str_Z[idx]) scr.Z += str_Z[idx] - val;
    });
    return scr;
}