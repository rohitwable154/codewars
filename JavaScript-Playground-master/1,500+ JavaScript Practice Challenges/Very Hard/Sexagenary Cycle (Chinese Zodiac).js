...
const sexagenary = year => ['Metal', 'Water', 'Wood', 'Fire', 'Earth'][(year >> 1) % 5] + ' ' + ['Monkey', 'Rooster', 'Dog', 'Pig', 'Rat', 'Ox', 'Tiger', 'Rabbit', 'Dragon', 'Snake', 'Horse', 'Sheep'][year % 12];
...
const c = (diff, len) => diff > 0 ? diff % len : (diff % len + len) % len;
...
function sexagenary(year) {
    ...
    diff = year - 1984
    return [stems[c(diff, 10)], branches[c(diff, 12)]].join(' ')
}
...
function sexagenary(year) {
    ...
    return s1[(year + 6) % 10] + ' ' + s2[(year + 8) % 12];
}