...
longestSlide = p =>
    p.reduceRight((a, x) => x.map((v, i) => v + Math.max(a[i], a[i + 1])))[0];
...
const longestSlide = pyramid => {
    ...
    while (pyramid.length > 0)
        res = pyramid.pop().map((a, i) => a + Math.max(res[i], res[i + 1]));
    return res[0];
}
...
Test Passed: Value == 23
Test Passed: Value == 1074
Test Passed: Value == 7273
