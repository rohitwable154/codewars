...
public class Program {
    static int[] PI = new int[] { 3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 9, 7, 9 };
    static int MAX_LEN = PI.Sum();
    public static string PilishString(string txt) {
        if (txt == "") return txt;
        txt = txt.Substring(0, Math.Min(txt.Length, MAX_LEN));
        var words = new List < string > ();
        int index = 0;
        foreach(var p in PI) {
            if (p > txt.Length - index) {
                txt += new string(txt.Last(), index + p - txt.Length);
            }
            words.Add(txt.Substring(index, p));
            index += p;
