...
let maximumScore=tileHand=>tileHand.map(x=>x.score).reduce((a,b)=>a+b)

maximumScore = object => object.map(a => a.score).reduce((acc, num) => acc + num)
...
function maximumScore(k) {
	return k.map(n => n.score).reduce((a,b) => a + b);
