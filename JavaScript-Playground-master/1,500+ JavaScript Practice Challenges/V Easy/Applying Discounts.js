...
const getDiscounts=(nums,d)=>nums.map(x =>x*Number(d.slice(0,-1))/100)

const getDiscounts = (nums, d) => nums.map(n => n * parseInt(d) / 100);

const getDiscounts = (arr, x) => arr.map(n => n * parseInt(x) / 100);

const getDiscounts = (nums, discount) =>   nums.map(num => num * parseInt(discount, 10) / 100);
...
{
    return nums.map(num => num * parseInt(d) / 100);
  }